...
# Fix for the tree visualization issue
# You need to create a separate decision tree model to visualize
# since you're already using RandomForest

# Create a decision tree model
tree = DecisionTreeClassifier(random_state=42)
# Train the decision tree on the same data
...
export_graphviz(tree, out_file=dot_data,
                ...
                class_names=['rice', 'maize', 'Soyabeans', 'beans', 'peas', 'groundnuts', 'cowpeas', 
                            'banana', 'mango', 'grapes', 'watermelon', 'apple', 'orange', 'cotton', 'coffee'])
graph = pydotplus.graph_from_dot_data(dot_data.getvalue())
graph.write_png('crops.png')
Image(graph.create_png())

# Fix for the user input testing section
# Convert input strings to float values
n = float(input("Enter the Nitrogen amount (0 to 250) in your soil __"))
p = float(input("Enter the potassium amount (0 to 250) in your soil __"))
k = float(input("Enter the phosphorous amount (0 to 250) in your soil __"))
temperature = float(input("Enter the average annual temperature for this year (degree celsius) 0 to 100 __"))
humidity = float(input("Enter the humidity of the region (0 to 100) __"))
ph = float(input("Enter the pH (Strength of Hydrogen) of your soil (0 to 14) __"))
rainfall = float(input("Enter the annual rainfall in mm (0 to 1000) __"))

# Create the test data in the correct format
set_test = np.array([n, p, k, temperature, humidity, ph, rainfall]).reshape(1, -1)

# Standardize the input using the same scaler
set_test_std = scaler.transform(set_test)

# Make a prediction using the trained RandomForest model (not rf_model_on_full_data)
prediction_result = rf_best_full.predict(set_test_std)[0]  # Note: using rf_best_full, not rf_model_on_full_data

# Print the predicted crop
print(f"Your suggested main plant is: {prediction_result}")

# For the OpenAI API section, use the prediction_result for label
label = prediction_result  # Use this for the crop manual generation
'''
"""Using ***OpenAI API*** to provide crop Manual."""

api_key = "OPENAI" #Replace this with your API key
